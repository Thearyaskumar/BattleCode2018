         ...
         for(int i = 0; i < units.size(); i++){
            ...
            if(!seen.contains(u.id())) {
               ...
               switch(u.unitType()){ // Add it to the respective collection
                  case Factory: myFactories.add(new Factory(u, gc)); 
                     break;
                  case Rocket:  myRockets.add(new Rocket(u, gc));    
                     break;
                  case Worker:  myWorkers.add(new Worker(u,gc));     
                     break;
                  case Mage:    myMages.add(new Mage(u,gc));         
                     break;
                  case Ranger:  myRangers.add(new Ranger(u,gc));     
                     break;
                  case Knight:  myKnights.add(new Knight(u,gc));     
                     break;
                  case Healer:  myHealers.add(new Healer(u,gc));     
                     break;
               }
            }
         }
            ...
            switch(r.getTarget()){ //-1-unbuilt, 0-unfull,1-full waiting, 2-launching
               ...
               case 0:  //sense all units nearby this rocket
                  ...
                  if (r.fullGar() || r.health()<40 || gc.round()>745)
                     try{r.setTarget(1);}
                     catch(Exception e){} //if it's now full, time to idle
                             //In Robot code, if type is what r wants, bug toward r's location and load.
                  break;
               case 1:  
                  for (int i = 0; i < 5; i++){ //don't do this more than 5 times per turn to avoid timeOut
                     if (gc.canLaunchRocket(r.id(),r.getLandingLoc()))
                        break;
                     else
                        try{r.findLandingLoc();}
                        catch(Exception e){}
               ...
               case 3:  
                  if (r.emptyGar()){
                     try{gc.disintegrateUnit(r.id());}
                     catch(Exception e){}
                              
                     try{myRockets.remove(r);}
                     catch(Exception e){}
                  }
                  else{ 
